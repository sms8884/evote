...
$(document).ready(function(){
	...
	var ss = "${voteInfo.start_date}";
	var ee = "${voteInfo.end_date}";
	var shour = ss.substring(ss.length-2, ss.length);
	var ehour = ee.substring(ee.length-2, ee.length);		
	...
	$("#btn_reg").on("click", function(e) {
		...
		$('#form').attr('action', "/admin/vote/voteMod").submit();		
	});
								...
								<tbody>
									<tr>
										...
										<td colspan="3">
											...
											<c:if test ="${voteInfo.vote_type eq 'ALL'}">일괄 투표</c:if>
							   				<c:if test ="${voteInfo.vote_type eq 'PART'}">분야별 투표</c:if>
										</td>
									</tr>
									<tr>
										<th scope="row">투표제목</th>
										<td colspan="3">
											<textarea rows="" cols="100" id="title" name="title" maxlength="30"><c:out value="${voteInfo.title}"/></textarea>
									...
									<tr>
										...
										<td colspan="3">
											...
											<fmt:parseDate value="${voteInfo.start_date}" var="sdateFmt" pattern="yyyyMMddHH"/>
											...
											<fmt:parseDate value="${voteInfo.end_date}" var="edateFmt" pattern="yyyyMMddHH" />
											<fmt:formatDate value="${edateFmt}"  pattern="yyyy.MM.dd" var="eDate"/>	
											<input type="text" class="it wid125" id="end_date" name="end_date" value="${eDate}" readOnly/><img src="${siteImgPath}/common/icon_cal.gif" id="img_ed"/><select  id="end_hour" name="end_hour"></select>시
										</td>
									</tr>
									<tr>
										<th scope="row">투표 대상</th>
										<td>
											<input type="radio" id="target1" name="target" value="ALL" <c:if test="${voteInfo.target eq 'ALL'}">checked</c:if>/><label for="target1">전체</label>
											<input type="radio" id="target2" name="target" value="YOUNG" <c:if test="${voteInfo.target eq 'YOUNG'}">checked</c:if>/><label for="target2">청소년</label>
 											<input type="radio" id="target3" name="target" value="ADULT" <c:if test="${voteInfo.target eq 'ADULT'}">checked</c:if>/><label for="target3">성인</label>
										</td>
										<th scope="row">투표 대상자</th>
										<td>
											<input type="text" class="it wid125" id="target_text" name="target_text" value="${voteInfo.target_text}" maxlength="100"/>
										</td>
									</tr>
									<tr>
										...
										<td colspan="3">
											<input type="radio" id="result_dp_yn_y" name="result_dp_yn" value="Y" <c:if test="${voteInfo.result_dp_yn eq 'Y'}">checked</c:if>/><label for="result_dp_yn_y">노출</label>
											<input type="radio" id="result_dp_yn_n" name="result_dp_yn" value="N" <c:if test="${voteInfo.result_dp_yn eq 'N'}">checked</c:if>/><label for="result_dp_yn_n">비노출</label>
		     								<p class="upload">
									...
									<tr>
										...
										<td style="border-right-style: none;" colspan="3">
											<div class="writeW">
												<textarea cols="10" rows="10" title="투표정보 입력" class="wid100" id="vote_info" name="vote_info" maxlength="2000" onkeyup="return checkmaxlength(this,'최대 2000자까지 입력가능합니다.');" >${voteInfo.vote_info}</textarea>
											</div>
									...
									<tr>
										...
										<td style="border-right-style: none;" colspan="3">
											<div class="writeW">
												<textarea cols="10" rows="10" title="투표결과 입력" class="wid100"  id="vote_result" name="vote_result" maxlength="2000" onkeyup="return checkmaxlength(this,'최대 2000자까지 입력가능합니다.');">${voteInfo.vote_result}</textarea>
											</div>
											<span class="red2">최대 2000자까지 입력가능</span>
										</td>
									</tr>									
