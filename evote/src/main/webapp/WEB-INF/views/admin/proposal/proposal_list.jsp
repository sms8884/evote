...
//검색
function fnSearch(){
	$('#form').attr('action', "/admin/proposal/proposal_list");
	gotoPage(1);
...
function fnMoveDetail(propSeq){
	$('#propSeq').val(propSeq);
	$('#listUrl').val( "/admin/proposal/proposal_list"); //디테일 화면에 상세보기에서 리스트로 갈 페이지
...
function sortList(sort){
	$("#sortItem").val(sort);
	$('#form').attr('action', "/admin/proposal/proposal_list").submit();		
}
				...
				<div class="content">
					...
					<!-- form에 클래스가 걸려 있어서 form위치 이동하면 안됨 -->	
								...
								<table cellpadding="0" cellspacing="0" class="" summary="" >
									...
									<tbody>
										<tr>
											...
											<td>
												<select title="구분" class="wid100" name="status" id="status">
													...
													<option value="<c:out value='${code.cdId }'/>" <c:if test ="${code.cdId eq param.status }">selected="selected"</c:if> ><c:out value="${code.cdNm }" /></option>
													</c:forEach>
												</select>
											</td>
										</tr>
										<tr>
											...
											<td colspan="3">
												<select title="검색" class="wid141" id="searchColumn" name="searchColumn">
												...
												<label for="con" class="hidden">검색내용 입력하기</label><input type="text" id="searchKeyword" name="searchKeyword" class="it wid461" title="" value="${param.searchKeyword}"/>
											</td>
										</tr>
									</tbody>
								</table>
						...
						<jsp:include page="/WEB-INF/views/admin/common/paging.jsp">
							...
							<jsp:param name="action" value="/admin/proposal/proposal_list"/>
						</jsp:include>	
						<!-- //paging -->		
