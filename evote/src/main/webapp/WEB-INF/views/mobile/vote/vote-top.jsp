...
<c:if test="${params.finish_yn eq 'Y'}">
	<!-- 투표참여 완료 -->
	<div class="sel_top"><span>선택 완료</span></div>
</c:if>
	<p class="tit" id="tmpTitle"><c:out value="${fn:replace(voteInfo.title, crcn, br)}" escapeXml="false"/></p>
...
<ul class="conTab two">
	<li><a href="#" <c:if test ="${tab_menu == 0}"> class="on"</c:if> onClick='javascript:goVoteList();' >제안목록(${voteTabCnt.total_cnt})</a></li>
	<li><a href="#" <c:if test ="${tab_menu == 1}"> class="on"</c:if> onClick='javascript:goVoteChoiceList();' >선택항목( ${voteTabCnt.user_choice_cnt} / ${voteTabCnt.vital_cnt} )</a></li>
</ul>
</c:if>
